...
# Preserve the microphone fix already validated in v4.0.
...
# Battery fix only: keep standard broadcast data, isolate all optional binder calls,
# and stop rebuilding the whole page every 180 ms.
...
battery='''    private void buildBatteryPage(){
        addHeading("Battery","État de la batterie lu via Android. Les mesures avancées non supportées sont simplement marquées indisponibles.");
    ...
    private void refreshBatteryPage(){
        ...
        try{
            ...
            batteryFields.removeAllViews();
            batteryFields.addView(Ui.muted(this,"Informations batterie avancées indisponibles sur cet appareil."));
        }
    }
...
s=s.replace('private void refreshVisiblePage(){if("Sensors".equals(page))refreshSensorsUi();else if("Battery".equals(page))refreshBatteryPage();}',
