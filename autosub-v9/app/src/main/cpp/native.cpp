...
Java_com_neo_autosub_localfull_MainActivity_nativeTranscribe(JNIEnv *env, jclass, jstring modelPath, jfloatArray samples) {
    ...
    // IMPORTANT: with whisper.cpp, detect_language=true means
    // "detect the language and exit". Keeping language="auto" while
    // detect_language=false performs automatic language detection and then
    // continues with the full transcription.
    p.detect_language = false;
    p.n_threads = std::max(2u, std::thread::hardware_concurrency());

    if (whisper_full(ctx, p, pcm.data(), (int)pcm.size()) != 0) {
        whisper_free(ctx);
        return env->NewStringUTF("{\"error\":\"Échec de la transcription locale\"}");
    }
    ...
    if (count <= 0) {
        whisper_free(ctx);
        return env->NewStringUTF("{\"error\":\"Whisper n'a détecté aucun texte parlé dans cette vidéo\"}");
    }

    std::ostringstream os;
    os << "{\"language\":\"" << esc(lang) << "\",\"segments\":[";
