...
public final class AirControlService extends Service implements Camera.PreviewCallback, MediaPipeHandTracker.Listener {
    ...
    private long lastFrameSubmit,lastUi,fpsWindow;
    ...
    private void createChannel(){
        if(Build.VERSION.SDK_INT>=26){
            NotificationChannel c=new NotificationChannel(CHANNEL,"AirControl camera",NotificationManager.IMPORTANCE_LOW);
    ...
    private int displayDegrees(){
        int r=((WindowManager)getSystemService(WINDOW_SERVICE)).getDefaultDisplay().getRotation();
        switch(r){case Surface.ROTATION_90:return 90;case Surface.ROTATION_180:return 180;case Surface.ROTATION_270:return 270;default:return 0;}
    }

    private int analysisRotationForFront(int id){
        Camera.CameraInfo info=new Camera.CameraInfo();Camera.getCameraInfo(id,info);
        int result=(info.orientation+displayDegrees())%360;
        return (360-result)%360;
    }

    private void startCamera(){
        cameraId=front();
        if(cameraId<0){mlError="aucune caméra avant";stopSelf();return;}
        try{
            ...
            Camera.Size a=camera.getParameters().getPreviewSize();fw=a.width;fh=a.height;
            rotation=analysisRotationForFront(cameraId);
            if(rotation==90||rotation==270){outW=192;outH=256;}else{outW=256;outH=192;}
            rgb=new int[outW*outH];
    ...
    private Camera.Size chooseSize(List<Camera.Size> sizes){
        ...
        Camera.Size best=null;long bestScore=Long.MAX_VALUE;
        for(Camera.Size s:sizes){
            long area=(long)s.width*s.height;
            long score=Math.abs(s.width-640L)*3+Math.abs(s.height-480L)*3+Math.abs(area-307200L)/200;
    ...
    @Override public void onPreviewFrame(byte[] data,Camera c){
        ...
        if(now-lastFrameSubmit<FRAME_INTERVAL_MS)return;
        lastFrameSubmit=now;
        nv21ToUprightRgb(data,fw,fh);
        frameBitmap.setPixels(rgb,0,outW,0,0,outW,outH);
        tracker.submit(frameBitmap);
    }
    ...
    private void tickFps(){
        ...
        if(now-fpsWindow>=1000){mlFps=resultFrames*1000f/Math.max(1,now-fpsWindow);resultFrames=0;fpsWindow=now;}
    }
    ...
    @Override public void onDestroy(){
        ...
        if(texture!=null)try{texture.release();}catch(Exception ignored){}
        if(tracker!=null)try{tracker.close();}catch(Exception ignored){}
