...
public final class MediaPipeHandTracker implements AutoCloseable {
    ...
    public boolean submit(Bitmap bitmap){
        ...
        try{MPImage image=new BitmapImageBuilder(bitmap).build();landmarker.detectAsync(image,ts);return true;}
        catch(RuntimeException e){busy.set(false);listener.onError("MediaPipe detect: "+(e.getMessage()==null?e.getClass().getSimpleName():e.getMessage()));return false;}
    }
