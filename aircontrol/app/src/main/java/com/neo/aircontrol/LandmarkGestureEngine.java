...
public final class LandmarkGestureEngine {
    ...
    private static final long COOLDOWN_MS=380, ACQUIRE_MS=90, HISTORY_MS=430;
    ...
    public void reset(){
        history.clear();haveFiltered=false;haveRaw=false;lastHandMs=firstHandMs=cooldownUntil=0;
        d.handFound=d.tracking=d.grace=false;d.samples=0;d.speed=d.displacement=d.coherence=0;
    ...
    public GestureCommand onNoHand(long now){
        ...
        if(gap<=HARD_LOST_MS){
            d.grace=true;
            d.tracking=haveRaw;
            d.state=gap<=SOFT_LOST_MS?"HOLD":"SEARCHING";
            return GestureCommand.NONE;
        }
        history.clear();haveFiltered=false;haveRaw=false;firstHandMs=0;
    ...
    public GestureCommand process(float[] x,float[] y,float[] z,long now){
        ...
        long gap=lastHandMs==0?Long.MAX_VALUE:now-lastHandMs;
        float rawDx=haveRaw?palmX-lastRawX:0f;
        float rawDy=haveRaw?palmY-lastRawY:0f;
        float rawAdx=Math.abs(rawDx),rawAdy=Math.abs(rawDy);
        ...
        boolean canBridge=haveRaw&&gap<=BRIDGE_MAX_GAP_MS&&now>=cooldownUntil
                &&scaleRatio>.48f&&scaleRatio<2.10f&&rawDist>=FAST_MIN&&rawDist<=FAST_MAX;
        GestureCommand bridge=GestureCommand.NONE;
        if(canBridge){
            if(rawAdx>rawAdy*FAST_DOMINANCE)bridge=rawDx>0?GestureCommand.RIGHT:GestureCommand.LEFT;
            else if(rawAdy>rawAdx*FAST_DOMINANCE)bridge=rawDy>0?GestureCommand.DOWN:GestureCommand.UP;
        }

        lastHandMs=now;lastRawX=palmX;lastRawY=palmY;lastRawScale=palmScale;haveRaw=true;
        ...
        if(bridge!=GestureCommand.NONE){
            fx=palmX;fy=palmY;haveFiltered=true;
            d.x=fx;d.y=fy;d.displacement=Math.max(rawAdx,rawAdy);
            d.speed=d.displacement/(Math.max(35,Math.min(BRIDGE_MAX_GAP_MS,gap))/1000f);
            d.coherence=1f;d.lastCommand=bridge;d.state="FAST_"+bridge.name();
            cooldownUntil=now+COOLDOWN_MS;
            history.clear();history.addLast(new Sample(fx,fy,now));d.samples=1;
            firstHandMs=now-ACQUIRE_MS;
            return bridge;
        }
        ...
        if(!haveFiltered){
            ...
            if(jump>.58f){
                ...
                float alpha;
                if(gap>SOFT_LOST_MS)alpha=.84f;
                else if(jump>.075f)alpha=.82f;
                else alpha=.58f;
        ...
        while(!history.isEmpty()&&(history.size()>22||now-history.peekFirst().t>HISTORY_MS))history.removeFirst();
        ...
        if(coherence<.62f){d.state="TRACKING";return GestureCommand.NONE;}

        d.lastCommand=candidate;d.state="FIRED_"+candidate.name();cooldownUntil=now+COOLDOWN_MS;
        history.clear();history.addLast(new Sample(fx,fy,now));firstHandMs=now-ACQUIRE_MS;
        return candidate;
    }
    ...
    private float coherence(GestureCommand cmd){
        ...
        return direction*.60f+straight*.40f;
    }
