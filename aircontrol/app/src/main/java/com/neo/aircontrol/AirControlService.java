...
public final class AirControlService extends Service implements Camera.PreviewCallback, MediaPipeHandTracker.Listener {
    ...
    private int resultFrames,lastVolumeBefore=-1,lastVolumeAfter=-1;
    private float mlFps;
    ...
    @Override public void onCreate(){
        ...
        createChannel();startForeground(42,notification("AirControl V7.4 · stable mouse + isolated volume"));
    ...
    @Override public void onHands(float[][] xs,float[][] ys,float[][] zs,long timestampMs,long inference){
        ...
        if(pr.active||pr.blockSwipes){volumeEngine.reset();engine.reset();handlePointer(pr);updatePointerDebug(pr,xs==null?0:xs.length);return;}
        VolumeGestureEngine.Result vr=volumeEngine.process(xs,ys,zs,timestampMs);
        if(vr.active||vr.blockOther){pointerEngine.reset();engine.reset();hideCursor();handleVolume(vr);updateVolumeDebug(vr);return;}
        hideCursor();
    ...
    private void handleVolume(VolumeGestureEngine.Result vr){
        if(audio==null||(!vr.volumeUp&&!vr.volumeDown))return;
        try{
            int max=audio.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
            int min=Build.VERSION.SDK_INT>=28?audio.getStreamMinVolume(AudioManager.STREAM_MUSIC):0;
            int before=audio.getStreamVolume(AudioManager.STREAM_MUSIC);
            int target=clamp(before+(vr.volumeUp?1:-1),min,max);
            lastVolumeBefore=before;
            if(target!=before)audio.setStreamVolume(AudioManager.STREAM_MUSIC,target,AudioManager.FLAG_SHOW_UI);
            lastVolumeAfter=audio.getStreamVolume(AudioManager.STREAM_MUSIC);
            bubbleState(9);
        }catch(Exception e){setStatus("VOLUME ERR · "+e.getClass().getSimpleName());}
    }
    ...
    private void updateVolumeDebug(VolumeGestureEngine.Result vr){long now=SystemClock.uptimeMillis();if(now-lastUi<100)return;bubbleState(9);String vol=lastVolumeBefore>=0?(" · vol "+lastVolumeBefore+"→"+lastVolumeAfter):"";String text=String.format(Locale.US,"MODE VOLUME · %s%s\npose %s · candidat %s · y %.2f · delta %.3f\n3 doigts verrouillent les swipes\nhaut = + / bas = -",vr.state,vol,vr.poseFound?"oui":"non",vr.candidate?"oui":"non",vr.handY,vr.deltaY);setStatus(text);}
