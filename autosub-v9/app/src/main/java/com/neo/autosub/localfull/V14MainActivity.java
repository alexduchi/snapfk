...
public class V14MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        updateGuard("AutoSub V15 actif en arrière-plan.");
    }
    ...
    private void buildUi(){
        ...
        TextView title=txt("AutoSub Local V15",28);title.setTypeface(null,1);root.addView(title);
        TextView d=txt("Aperçu + taille et position des sous-titres réglables avant export",14);d.setTextColor(0xffaeb8d0);root.addView(d);
        ...
        preview=new VideoView(this);previewFrame.addView(preview,new FrameLayout.LayoutParams(-1,-1));
    ...
    private void setControlsVisible(boolean visible){
        ...
        sizeLabel.setVisibility(v);sizeSeek.setVisibility(v);xLabel.setVisibility(v);xSeek.setVisibility(v);yLabel.setVisibility(v);ySeek.setVisibility(v);resetPositionButton.setVisibility(v);exportButton.setVisibility(v);
    }
    ...
    private void selectVideo(Uri u,String name){
        videoUri=u;preparedSubs.clear();previewHandler.removeCallbacksAndMessages(null);subtitlePreview.setText("");subtitlePreview.setVisibility(View.GONE);setControlsVisible(false);selected.setText("Vidéo sélectionnée : "+(name==null?u.getLastPathSegment():name));readVideoDimensions(u);preview.setVideoURI(u);preview.setMediaController(new MediaController(this));preview.start();detected.setText("Langue détectée : —");status.setText("Vidéo chargée : "+videoWidth+"×"+videoHeight+". Génère l'aperçu.");
    }
    ...
    private void runPipeline(){
        ...
        process.setEnabled(false);setControlsVisible(false);subtitlePreview.setVisibility(View.GONE);setBusy(true);status.setText("Extraction audio locale…");updateGuard("Extraction audio locale…");final int targetIndex=targetSpinner.getSelectedItemPosition();
    ...
    private void applyPreviewLayout(){
        float ratio=videoWidth/(float)Math.max(1,videoHeight);float baseSp=ratio<0.68f?16f:(ratio<1.0f?18f:21f);subtitlePreview.setTextSize(baseSp*subtitleScale);subtitlePreview.setMaxLines(ratio<1.0f?4:3);applyPreviewPosition();
    }

    private void applyPreviewPosition(){
        if(previewFrame==null||subtitlePreview==null)return;
        previewFrame.post(()->{
            int fw=previewFrame.getWidth(),fh=previewFrame.getHeight();if(fw<=0||fh<=0)return;
            float videoAspect=videoWidth/(float)Math.max(1,videoHeight),frameAspect=fw/(float)Math.max(1,fh);float contentW,contentH;
            if(videoAspect>frameAspect){contentW=fw;contentH=fw/videoAspect;}else{contentH=fh;contentW=fh*videoAspect;}
            int maxTextWidth=(int)Math.max(dp(80),contentW*(videoAspect<0.68f?0.76f:0.88f));subtitlePreview.setMaxWidth(maxTextWidth);
            subtitlePreview.measure(View.MeasureSpec.makeMeasureSpec(maxTextWidth,View.MeasureSpec.AT_MOST),View.MeasureSpec.makeMeasureSpec((int)contentH,View.MeasureSpec.AT_MOST));
            float sw=subtitlePreview.getMeasuredWidth(),sh=subtitlePreview.getMeasuredHeight();float maxX=Math.max(0f,(contentW-sw)/2f-dp(4)),maxY=Math.max(0f,(contentH-sh)/2f-dp(4));
            float nx=(Math.max(0,Math.min(100,subtitleXPercent))-50)/50f;float ny=(Math.max(0,Math.min(100,subtitleYPercent))-50)/50f;
            subtitlePreview.setTranslationX(nx*maxX);subtitlePreview.setTranslationY(ny*maxY);
        });
    }
