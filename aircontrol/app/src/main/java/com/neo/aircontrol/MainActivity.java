...
public final class MainActivity extends Activity {
    ...
    private View buildUi(){
        ...
        TextView title=new TextView(this);title.setText("AirControl V5");title.setTextSize(30);title.setTextColor(0xFF111318);title.setPadding(0,dp(18),0,dp(6));root.addView(title);
        TextView sub=new TextView(this);sub.setText("Suivi automatique renforcé\n\n1. Autorise caméra + bulle flottante.\n2. Active AirControl dans Accessibilité.\n3. Vérifie avec TEST SWIPE.\n4. Lance ON.\n\nLa détection tourne sans V ni armement.");sub.setTextSize(16);sub.setTextColor(0xFF40444B);root.addView(sub);
        ...
        Button test=new Button(this);test.setText("TEST SWIPE BAS");test.setOnClickListener(v->{boolean ok=AirAccessibilityService.perform(GestureCommand.DOWN);Toast.makeText(this,ok?"Swipe demandé · "+AirAccessibilityService.lastGestureStatus():"Service accessibilité OFF",Toast.LENGTH_LONG).show();refresh();});root.addView(test,lp());
        ...
        state=new TextView(this);state.setPadding(0,dp(16),0,0);state.setTextSize(15);root.addView(state);refresh();return scroll;
    }
    ...
    private void refresh(){if(state!=null){String acc=AirAccessibilityService.state(this);state.setText("Accessibilité détectée : "+acc+"\nService connecté : "+(AirAccessibilityService.isReady()?"OUI":"NON")+"\nDernier geste : "+AirAccessibilityService.lastGestureStatus()+"\nOverlay : "+(Build.VERSION.SDK_INT<23||Settings.canDrawOverlays(this)?"OK":"à autoriser"));state.setTextColor(AirAccessibilityService.isReady()?0xFF1B5E20:0xFFB71C1C);}}
